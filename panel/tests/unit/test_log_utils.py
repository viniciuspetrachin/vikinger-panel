...
def test_normalize_docker_log_line_reformats_supervisord_prefix():
    ...
    assert result == "[Jul  7 00:45:19] [valheim-updater] Success! App '896660' fully installed."
...
def test_normalize_docker_log_line_strips_caret_inline():
    ...
    assert normalize_docker_log_line(raw) == "[Jul  7 12:30:09] [valheim-updater] Loading Steam API..."
...
def test_normalize_docker_log_line_strips_caret_merged():
    ...
    assert normalize_docker_log_line(raw) == "[Jul  7 12:30:10] [valheim-updater] Waiting for client config..."
...
def test_clean_docker_logs_filters_empty_and_normalizes():
    ...
    assert "[valheim-updater] Success!" in cleaned
...
def test_clean_docker_logs_steamcmd_block():
    ...
    assert "[valheim-updater] Loading Steam API..." in cleaned
    assert "[valheim-updater] Waiting for client config..." in cleaned
    assert "[valheim-updater] Success! App '896660' fully installed." in cleaned
    assert "[valheim-updater] INFO - Valheim Server is already the latest version" in cleaned
    assert cleaned.count("[valheim-updater] OK") == 2
