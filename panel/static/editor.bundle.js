...
// editor-src.js
var DRAFT_PREFIX = "valheim-panel:draft:";
var valheimTheme = EditorView.theme({
  "&": {
...
function saveDraft(path, content2) {
  try {
    localStorage.setItem(draftKey(path), JSON.stringify({ content: content2, ts: Date.now() }));
  } catch {
  }
}
...
var PanelEditorInstance = class {
  constructor(container, options) {
    ...
    if (options.minHeight) {
      container.style.minHeight = typeof options.minHeight === "number" ? `${options.minHeight}px` : options.minHeight;
    }
    this._notifyDirty();
    this._scheduleDraft();
  }
  ...
  setContent(content2, { markSaved = false, path = null } = {}) {
    ...
    if (cur !== content2) {
      this.view.dispatch({ changes: { from: 0, to: cur.length, insert: content2 } });
    }
    if (markSaved) {
      this.savedContent = content2;
